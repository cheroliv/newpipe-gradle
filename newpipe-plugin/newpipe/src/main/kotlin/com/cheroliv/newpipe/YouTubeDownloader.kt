...
class YouTubeDownloader : VideoInfoProvider {
    ...
    override suspend fun getVideoInfo(url: String): VideoMetadata = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val ageResult = ageVerificationHandler.checkAgeRestriction(e)
            if (ageResult.isAgeRestricted) {
                logger.warn("Age-restricted video detected: ${e.message}")
                ageVerificationHandler.throwAgeRestrictedException(ageResult, url, e)
            }

...
class DownloaderImpl private constructor(
    ...
    private fun doExecute(request: NewPipeRequest, session: Session?): NewPipeResponse {
        ...
        if (session != null && (response.code == 401 || response.code == 403)) {
            val errorResult = authErrorHandler.handleHttpError(response.code, session.id)
            sessionManager?.markInvalid(session.id, errorResult)
            authErrorHandler.logError(errorResult)
            response.close()

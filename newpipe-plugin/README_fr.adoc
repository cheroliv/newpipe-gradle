...
<p align="right">
  <a href="README.adoc">
    <img src="images/lang-en-blue.svg" alt="English" width="32"/>
  </a>
</p>
++++
...
└── newpipe/
    └── src/
        ...
        │   ├── DownloaderPlugin.kt      Point d'entrée du plugin
        │   ├── NewpipeExtension.kt      Extension DSL (configPath)
        │   ├── NewpipeManager.kt        Enregistrement des tâches + parsing YAML
        │   ├── Models.kt                Data classes Selection / Artist
...
DownloadMusicTask.download()
    ...
    │           ├── Mix Radio (list=RD...) → première page (~15 pistes)
    ...
    └── Pipeline concurrent (Semaphore 3) pour chaque URL
          ...
          │
          └── convertTrack() — immédiatement après le download
...
Les fakes retournent des données fixes instantanément — aucun appel réseau,
aucun FFmpeg. Les tests vérifient la structure des dossiers, le nommage des
fichiers, le skip des doublons et la gestion des erreurs en moins d'une
seconde par scénario.
...
pointe sur la branche `dev` pour bénéficier des derniers correctifs
anti-throttling YouTube.
====
...
de l'URL de flux. En cas de lenteur anormale sur une connexion fibre, bumper
la dépendance vers le dernier commit `dev` de NewPipe Extractor est la
première action à entreprendre.

=== Mix Radio (list=RD...)

Les playlists Mix Radio sont générées dynamiquement par YouTube et ne peuvent
pas être paginées au-delà de la première page. La pagination complète n'est
pas possible sans l'API officielle YouTube.
